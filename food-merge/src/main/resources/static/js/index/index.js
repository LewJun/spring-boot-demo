        ...
        {title: '操作', align: 'center',
            events: {
                'click #fd-detail': function(e, value, row, index) {
                    // 根据fd001查询基础数据，和合并后的数据
                    initDetailFdTable(row.fd001)
                    // 显示到fdDetailModal中的相应位置
                    initDetailYyssTable(row.yyss001)
                    // 显示模态框
...
$("#yyssTable").bootstrapTable('destroy');
...
function initDetailFdTable(fd001) {
$("#detailFdTable").bootstrapTable('destroy');
if(!fd001) return;

$("#detailFdTable").bootstrapTable({
    url: "fd/"+fd001,   //请求地址
    striped: true, //是否显示行间隔色
    pageNumber: 1, //初始化加载第一页
...
function initDetailYyssTable(yyss001) {
$("#detailYyssTable").bootstrapTable('destroy');
if(!yyss001) return;

$("#detailYyssTable").bootstrapTable({
    url:"yyss/" + yyss001,   //请求地址
    striped: true, //是否显示行间隔色
    pageNumber: 1, //初始化加载第一页
    pagination: false, //是否分页
    sidePagination: 'server',//server:服务器端分页|client：前端分页
